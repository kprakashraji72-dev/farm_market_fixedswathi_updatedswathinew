...
def _load_rsa_private_key():
    """
    Loads RSA private key from settings.FLOW_PRIVATE_KEY_PEM.
    Supports PEM string directly, or reads from file path if specified.
    Supports optional passphrase from settings.FLOW_PRIVATE_KEY_PASSPHRASE.
    """
    pem_raw: str = getattr(settings, "FLOW_PRIVATE_KEY_PEM", "") or ""
    passphrase: str | None = getattr(settings, "FLOW_PRIVATE_KEY_PASSPHRASE", None)

    password_bytes = passphrase.encode("utf-8") if passphrase else None

    # 1. If PEM raw is empty, try loading from default or configured file path
    if not pem_raw.strip():
        key_path = getattr(settings, "FLOW_PRIVATE_KEY_PATH", "whatsapp_flow_private.pem")
        if os.path.exists(key_path):
            with open(key_path, "rb") as f:
                return load_pem_private_key(f.read(), password=password_bytes)
        raise FlowCryptoError(
            "FLOW_PRIVATE_KEY_PEM is empty and no valid private key file was found."
        )

    # 2. Convert literal escaped \n to real newlines if present
    formatted_pem = pem_raw.replace("\\n", "\n").strip().encode("utf-8")
    return load_pem_private_key(formatted_pem, password=password_bytes)
